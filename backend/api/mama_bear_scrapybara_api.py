"""
🐻 Mama Bear Scrapybara API - Scout-Level Endpoints
Exposes all Scout capabilities through REST API for Mama Bear
"""
...
try:
    from services.mama_bear_scrapybara_integration import (
        MamaBearScrapybaraAgent,
        create_mama_bear_scrapybara_agent,
        integrate_with_mama_bear_orchestrator
    )
    SCRAPYBARA_AVAILABLE = True
except ImportError as e:
    logger.warning(f"Scrapybara integration not available: {e}")
    SCRAPYBARA_AVAILABLE = False
    MamaBearScrapybaraAgent = None

# Create blueprint
mama_bear_scrapybara_bp = Blueprint('mama_bear_scrapybara', __name__, url_prefix='/api/mama-bear-scrapybara')

# Global agent instance
scrapybara_agent: Optional[MamaBearScrapybaraAgent] = None
...
async def get_scrapybara_agent() -> Optional[MamaBearScrapybaraAgent]:
    ...
    if scrapybara_agent is None and SCRAPYBARA_AVAILABLE:
        try:
            ...
            scrapybara_agent = await create_mama_bear_scrapybara_agent(config)
...
@mama_bear_scrapybara_bp.route('/web-search', methods=['POST'])
def web_search():
...
@mama_bear_scrapybara_bp.route('/browse-website', methods=['POST'])
def browse_website():
...
@mama_bear_scrapybara_bp.route('/execute-code', methods=['POST'])
def execute_code():
...
@mama_bear_scrapybara_bp.route('/copycapy-analyze', methods=['POST'])
...
@mama_bear_scrapybara_bp.route('/generate-image', methods=['POST'])
def generate_image():
...
@mama_bear_scrapybara_bp.route('/file-operations', methods=['POST'])
def file_operations():
...
@mama_bear_scrapybara_bp.route('/download-file', methods=['POST'])
def download_file():
...
@mama_bear_scrapybara_bp.route('/start-collaboration', methods=['POST'])
def start_collaboration():
    """Start collaborative session between user and Mama Bear"""
    try:
        data = request.get_json()
        user_id = data.get('user_id', 'anonymous')
...
@mama_bear_scrapybara_bp.route('/scout-workflow', methods=['POST'])
def scout_workflow():
...
@mama_bear_scrapybara_bp.route('/capabilities', methods=['GET'])
...
@mama_bear_scrapybara_bp.route('/task-history', methods=['GET'])
...
@mama_bear_scrapybara_bp.route('/active-sessions', methods=['GET'])
...
@mama_bear_scrapybara_bp.route('/memory/store', methods=['POST'])
def store_memory():
...
@mama_bear_scrapybara_bp.route('/memory/search', methods=['POST'])
def search_memory():
...
@mama_bear_scrapybara_bp.route('/memory/stats', methods=['GET'])
...
@mama_bear_scrapybara_bp.route('/health', methods=['GET'])
def health_check():
...
def integrate_mama_bear_scrapybara_api(app):
    """Integrate Mama Bear Scrapybara API with Flask app"""
    try:
        app.register_blueprint(mama_bear_scrapybara_bp)
        logger.info("🐻 Mama Bear Scrapybara API integrated successfully")
        
        # Initialize agent on startup
        with app.app_context():
            if SCRAPYBARA_AVAILABLE:
                logger.info("🚀 Initializing Mama Bear Scrapybara agent...")
        
        return True
        
    except Exception as e:
        logger.error(f"Failed to integrate Mama Bear Scrapybara API: {e}")
        return False